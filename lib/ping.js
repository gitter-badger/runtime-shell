...
module.exports = function(runtime) {
  runtime.shell.setCommand('ping', function(args, cb) {
    ...
    const dns = runtime.dns;
    const url = require('url');
    dns.resolve(url.parse(argarr[1]).host, function(err, resdata) {
      if (err) {
      ...
      for (var i = 0; i < times; i++) {
        http.get(argarr[1], function(res) {
          runtime.tty.print('statcode ' + res.statusCode + 'from ' + resdata.results[0].hostname + ' (' + resdata.results[0].address + '); ttl=' + resdata.results[0].ttl);
          if (i == times) {
            cb();
          }
        });
